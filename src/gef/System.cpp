...
void System::loadModule(const char* path) {
    ...
    GetMetadataFunc get_metadata = reinterpret_cast<GetMetadataFunc>(
        loader_->getSymbol(handle, "gef_get_metadata")
    );
    ...
    registry_->registerModule(metadata);
}

void System::executeModule(const char* name, Context&) {
    if (!name) {
        throw std::invalid_argument("Module name cannot be null");
    }
    
    const gef_metadata_t* metadata = registry_->getModule(name);
    if (!metadata) {
